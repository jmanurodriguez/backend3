...
describe('Adoptions Router - Functional Tests', () => {
  ...
  describe('GET /api/adoptions/:aid', () => {
    ...
    it('should return 404 for invalid adoption ID format', async () => {
      ...
      try {
        ...
        const response = await request(app).get(`/api/adoptions/${invalidId}`);
        response.status.should.be.oneOf([400, 404, 500]);
      }
    });
  });
  ...
  describe('POST /api/adoptions/:uid/:pid', () => {
    ...
    it('should return 404 for invalid user ID format', async () => {
      ...
      try {
        const response = await request(app).post(`/api/adoptions/${invalidUserId}/${petId}`);
        response.status.should.be.oneOf([400, 404, 500]);
      } catch (error) {
        const response = await request(app).post(`/api/adoptions/${invalidUserId}/${petId}`);
        response.status.should.be.oneOf([400, 404, 500]);
    ...
    it('should return 404 for invalid pet ID format', async () => {
      const usersRes = await request(app).get('/api/users');
      const userId = pickFirstId(usersRes.body.payload);
      const invalidPetId = 'invalid-pet-id';
      
      try {
        const response = await request(app).post(`/api/adoptions/${userId}/${invalidPetId}`);
        response.status.should.be.oneOf([400, 404, 500]);
      } catch (error) {
        const response = await request(app).post(`/api/adoptions/${userId}/${invalidPetId}`);
        response.status.should.be.oneOf([400, 404, 500]);
